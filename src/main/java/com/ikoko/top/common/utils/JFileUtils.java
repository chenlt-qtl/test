...
public class JFileUtils extends FileUtils {
	...
	public static boolean copyFile(String srcFileName, String descFileName) {
		return JFileUtils.copyFileCover(srcFileName, descFileName, false);
	}
	...
	public static boolean copyFileCover(String srcFileName,
		...
		if (descFile.exists()) {
			...
			if (coverlay) {
				...
				if (!JFileUtils.delFile(descFileName)) {
	...
	public static boolean copyDirectory(String srcDirName, String descDirName) {
		return JFileUtils.copyDirectoryCover(srcDirName, descDirName, false);
	}
	...
	public static boolean copyDirectoryCover(String srcDirName,
		...
		if (descDir.exists()) {
			if (coverlay) {
				...
				if (!JFileUtils.delFile(descDirNames)) {
		...
		if (files != null)
			for (int i = 0; i < files.length; i++) {
				// 如果是一个单个文件，则直接复制
				if (files[i].isFile()) {
					flag = JFileUtils.copyFile(files[i].getAbsolutePath(),
	...
	public static boolean delFile(String fileName) {
		...
		if (!file.exists()) {
			...
			if (file.isFile()) {
				return JFileUtils.deleteFile(fileName);
			} else {
				return JFileUtils.deleteDirectory(fileName);
			}
		}
	}
	...
	public static boolean deleteDirectory(String dirName) {
		...
		if (files != null)
			for (int i = 0; i < files.length; i++) {
				// 删除子文件
				if (files[i].isFile()) {
					flag = JFileUtils.deleteFile(files[i].getAbsolutePath());
					// 如果删除文件失败，则退出循环
					if (!flag) {
						break;
					}
				...
				// 删除子目录
					...
					flag = JFileUtils.deleteDirectory(files[i]
							.getAbsolutePath());
					// 如果删除子目录失败，则退出循环
					if (!flag) {
						break;
	...
	public static void writeToFile(String fileName, String content,
			boolean append) {
		try {
			JFileUtils.write(new File(fileName), content, "utf-8", append);
	...
	public static void writeToFile(String fileName, String content,
		...
		try {
			JFileUtils.write(new File(fileName), content, encoding, append);
			log.debug("文件 " + fileName + " 写入成功!");
		} catch (IOException e) {
			log.debug("文件 " + fileName + " 写入失败! " + e.getMessage());
		}
	...
	public static void zipFiles(String srcDirName, String fileName,
		...
		try {
			...
			if ("*".equals(fileName) || "".equals(fileName)) {
				JFileUtils.zipDirectoryToZipFile(dirPath, fileDir, zouts);
			} else {
				File file = new File(fileDir, fileName);
				if (file.isFile()) {
					JFileUtils.zipFilesToZipFile(dirPath, file, zouts);
				} else {
					JFileUtils.zipDirectoryToZipFile(dirPath, file, zouts);
				}
			}
			zouts.close();
			...
			if (files != null)
				for (int i = 0; i < files.length; i++) {
					if (files[i].isFile()) {
						...
						// 如果是目录，则递归调用
						JFileUtils.zipDirectoryToZipFile(dirPath, files[i],
								zouts);
					}
				}
